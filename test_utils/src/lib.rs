...
macro_rules! create_vm {
    ($vm_name:ident, $verified_executable:expr, $context_object:expr, $stack:ident, $heap:ident, $additional_regions:expr, $cow_cb:expr) => {
        let mut $stack = trezoa_rbpf::aligned_memory::AlignedMemory::zero_filled(
            $verified_executable.get_config().stack_size(),
        );
        let mut $heap = trezoa_rbpf::aligned_memory::AlignedMemory::with_capacity(0);
        ...
        .unwrap();
        let mut $vm_name = trezoa_rbpf::vm::EbpfVm::new(
